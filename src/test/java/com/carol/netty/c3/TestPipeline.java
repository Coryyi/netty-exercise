                    ...
                    protected void initChannel(NioSocketChannel nioSocketChannel) throws Exception {
                        ...
                        pipeline.addLast("h1",new ChannelInboundHandlerAdapter(){
                            ...
                            public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
                                ...
                                log.debug("1");
                                super.channelRead(ctx,msg);// 内部调用 ctx.fireChannel 也就是调用下一个handler 并且将处理结果传递给h2
                        ...
                        pipeline.addLast("h3",new ChannelInboundHandlerAdapter(){
                            ...
                            public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
                                ...
                                ctx.writeAndFlush(ctx.alloc().buffer().writeBytes("hello".getBytes()));// 直接ctx调用
                                // 注意 这里调用后还是不会执行出站handler 这是因为ctx只会从当前位置向前调用handler，倒着找 下一个时h2，所以没有从tail开始找
                                // 而channel的writeAndFlush()是从tail开始寻找
